-- ============================================
-- LLM Cost breakdown: normalize empty operation to 'other'
-- Avoids empty or null keys in by_operation JSONB
-- ============================================
...
BEGIN
    ...
    WITH requests AS (
        SELECT
            provider,
            model,
            COALESCE(NULLIF(TRIM(operation), ''), 'other') AS op_key,
    ...
    operation_agg AS (
        SELECT COALESCE(
            (SELECT jsonb_object_agg(op_key, ROUND(cost::numeric, 8))
             FROM (SELECT op_key, SUM(cost) AS cost FROM requests GROUP BY op_key) t),
            '{}'::jsonb
        ) AS j
    ),
    context_agg AS (
...
COMMENT ON FUNCTION get_llm_cost_breakdown_for_period IS 'Aggregated LLM cost breakdown for a date range; empty operation normalized to other';
