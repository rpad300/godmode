...
export function GraphStatusBar() {
    ...
    const connectionState = useMemo(() => {
        ...
        return { label: 'Disconnected', color: 'text-slate-400', icon: WifiOff, animate: false };
    ...
    return (
        <div className="absolute bottom-4 left-4 z-10 flex flex-col gap-2 w-[340px]">
            {/* Main Status Card */}
            <div className="bg-[var(--gm-bg-primary)] backdrop-blur-sm border rounded-lg shadow-sm p-3 text-xs flex flex-col gap-3">
                ...
                <div className="grid grid-cols-3 gap-2 py-1">
                    <div className="flex flex-col items-center p-2 bg-white/5 rounded border border-white/5">
                        <Database className="h-3 w-3 text-slate-400 mb-1" />
                        <span className="text-lg font-bold text-white">{status?.node_count || 0}</span>
                        <span className="text-[10px] text-slate-400 uppercase">Nodes</span>
                    </div>
                    <div className="flex flex-col items-center p-2 bg-white/5 rounded border border-white/5">
                        <Activity className="h-3 w-3 text-slate-400 mb-1" />
                        <span className="text-lg font-bold text-white">{status?.edge_count || 0}</span>
                        <span className="text-[10px] text-slate-400 uppercase">Edges</span>
                    </div>
                    <div className="flex flex-col items-center p-2 bg-white/5 rounded border border-white/5">
                        <Server className="h-3 w-3 text-slate-400 mb-1" />
                        <span className="text-lg font-bold text-white">{status?.pending_count || 0}</span>
