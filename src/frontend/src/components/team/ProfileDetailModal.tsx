              ...
              <ul className="space-y-1">
                {limitations.map((l, i) => (
                  <li key={i} className="text-xs text-gm-text-tertiary flex items-start gap-2">
                    <span className="text-gray-500 mt-0.5">•</span> {l}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        {cooperation.length > 0 && (
          <div className="bg-gm-surface-secondary rounded-lg p-3">
            ...
            <ul className="space-y-0.5">{cooperation.map((c, i) => <li key={i} className="text-xs text-gm-text-primary">→ {c}</li>)}</ul>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="space-y-2">
        {power.map((p, i) => (
          <div key={i} className="bg-gm-surface-secondary rounded-lg p-3">
            ...
            {p.strategic_implication && <p className="text-xs text-gm-interactive-primary mt-1">→ {String(p.strategic_implication)}</p>}
          </div>
        ))}
      </div>
    </div>
