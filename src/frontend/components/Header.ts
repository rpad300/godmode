...
function createProjectSelector(onProjectChange?: (projectId: string) => void): HTMLElement {
  ...
  const button = createElement('button', {
    className: 'gm-btn gm-btn--secondary project-btn',
  });
...
function createUserMenu(props: HeaderProps): HTMLElement {
  ...
  const button = createElement('button', { className: 'gm-btn gm-btn--ghost user-menu-btn' });
  ...
  function updateDropdown(): void {
    ...
    dropdown.innerHTML = `
      <div class="user-dropdown-header">
        ...
        ${user?.role ? `<span class="badge badge--pill badge--primary role-badge ${user.role}">${user.role}</span>` : ''}
      </div>
