...
function renderCosts(
  ...
  const budgetHtml =
    ...
    <div class="costs-budget-bar ${data.budgetAlertTriggered ? 'alert' : ''}">
      <div class="costs-budget-bar-fill" data-width="${Math.min(100, data.budgetUsedPercent)}"></div>
  ...
  if (billingSummary) {
    const balanceDisplay = billingSummary.unlimited_balance 
      ? '<span class="billing-balance billing-balance--unlimited">∞ Unlimited</span>'
      : formatEur(billingSummary.balance_eur);
    
    const statusBadge = billingSummary.unlimited_balance
      ? '<span class="badge badge-success badge--xs">Unlimited</span>'
      : billingSummary.balance_eur <= 0
        ? '<span class="badge badge-danger badge--xs">Blocked</span>'
        : billingSummary.balance_percent_used >= 80
          ? '<span class="badge badge-warning badge--xs">Low Balance</span>'
      ...
      <div class="billing-summary-banner">
        <div class="billing-summary-left">
          ...
          <div class="billing-summary-item">
            ...
            <div class="billing-summary-value">${statusBadge}</div>
          </div>
          ${billingSummary.current_tier_name ? `
          <div class="billing-summary-item">
            <div class="billing-summary-label">Current Tier</div>
            <div class="billing-summary-value billing-summary-tier">${billingSummary.current_tier_name} (+${billingSummary.current_markup_percent}%)</div>
          </div>
          ` : ''}
        </div>
  ...
  // Apply dynamic sizes without embedding style="..." in templates
  container.querySelectorAll<HTMLElement>('.costs-budget-bar-fill[data-width]').forEach(el => {
    const w = Number(el.dataset.width || '0');
    el.style.width = `${Math.max(0, Math.min(100, w))}%`;
  });

  container.querySelectorAll<HTMLElement>('.daily-chart .bar[data-height]').forEach(el => {
        ...
        ${chartDaily.map(d => {
          ...
          return `
            <div class="bar-column" title="${d.date}: ${formatCurrency(d.cost)} (${d.calls} calls)">
              <div class="bar ${isPlaceholder && d.cost === 0 ? 'bar-placeholder' : ''}" data-height="${height}"></div>
