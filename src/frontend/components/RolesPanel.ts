  ...
  container.innerHTML = `
    ...
    <div class="roles-panel-sota">
      ...
      <div class="roles-layout">
        ...
        <div class="role-editor gm-hidden" id="role-editor">
    ...
    if (currentRoles.length === 0) {
      listEl.innerHTML = `
        <div class="empty-state">
          ...
          <p class="roles-empty-hint">Click "Create Role" to get started</p>
        </div>
      `;
      return;
    }
    ...
    return `
      <div class="role-card${editingRole?.id === role.id ? ' active' : ''}" data-id="${role.id}">
        <div class="role-card-header">
          <div class="role-color-dot" style="--role-color: ${role.color || 'var(--color-primary)'}"></div>
...
function showRoleEditor(container: HTMLElement, role: Role | null): void {
  ...
  editorEl.classList.remove('gm-hidden');
    ...
    <div class="role-editor-body">
      ...
      <form id="role-form">
        <div class="form-row">
          ...
          <div class="form-group form-group--w-80">
            <label>Color</label>
            <input type="color" id="role-color" value="${role?.color || '#e11d48'}" class="role-color-input">
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="ai-enhance-section">
            ...
            <span class="form-hint form-hint--inline">Let AI generate or improve the role context based on the role name.</span>
          </div>
        </div>
...
function bindEditorEvents(container: HTMLElement, role: Role | null): void {
  ...
  if (cancelBtn) {
    on(cancelBtn as HTMLElement, 'click', () => {
      editorEl.classList.add('gm-hidden');
      editingRole = null;
      const listEl = container.querySelector('#roles-list') as HTMLElement;
      if (listEl) {
        listEl.querySelectorAll('.role-card').forEach(c => c.classList.remove('active'));
