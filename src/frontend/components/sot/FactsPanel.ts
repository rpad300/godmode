...
function renderFacts(container: HTMLElement, facts: Fact[], props: FactsPanelProps): void {
  ...
  // Apply dynamic category bar colors without embedding inline styles
  container.querySelectorAll<HTMLElement>('.fact-card-sota[data-bar-color]').forEach(card => {
    const c = card.dataset.barColor || '';
    if (!c) return;
    card.style.setProperty('--fact-category-bar', c);
    const bar = card.querySelector<HTMLElement>('.fact-category-bar');
...
function createFactCard(fact: Fact): string {
  ...
  return `
    <div class="fact-card-sota question-card-sota ${fact.verified ? 'has-answer' : ''}" data-id="${fact.id}" data-bar-color="${barColor}">
