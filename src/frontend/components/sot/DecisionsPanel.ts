...
function createConflictCard(c: DecisionConflict, panel: HTMLElement, props: DecisionsPanelProps): string {
  ...
  return `
    <div class="conflict-card-sota" data-decision-id1="${d1.id}" data-decision-id2="${d2.id}">
      <div class="conflict-card-priority-bar"></div>
...
function bindDecisionCards(container: HTMLElement, decisions: Decision[], props: DecisionsPanelProps): void {
  // Apply dynamic status bar colors without embedding inline styles
  container.querySelectorAll<HTMLElement>('.decision-card-sota[data-bar-color]').forEach(card => {
    const c = card.dataset.barColor || '';
    if (!c) return;
    card.style.setProperty('--decision-status-bar', c);
    const bar = card.querySelector<HTMLElement>('.decision-status-bar');
...
/** Status bar color (left edge) - align with token palette */
const STATUS_BAR_COLOR: Record<string, string> = {
  ...
  superseded: 'var(--color-secondary)',
...
function createDecisionCard(decision: Decision, contacts: ContactLike[]): string {
  ...
  return `
    <div class="decision-card-sota question-card-sota" data-id="${decision.id}" data-bar-color="${barColor}">
