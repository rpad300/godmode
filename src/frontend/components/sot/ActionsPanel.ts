...
function renderActions(container: HTMLElement, actions: Action[], props: ActionsPanelProps): void {
  ...
  container.innerHTML = actions.map(action => {
    ...
    return `
      <div class="action-card-sota question-card-sota ${isOverdue ? 'overdue' : ''}" data-id="${action.id}" data-bar-color="${barColor}">
  ...
  // Apply dynamic priority bar colors without embedding inline styles
  container.querySelectorAll<HTMLElement>('.action-card-sota[data-bar-color]').forEach(card => {
    const c = card.dataset.barColor || '';
    if (!c) return;
    card.style.setProperty('--action-priority-bar', c);
    const bar = card.querySelector<HTMLElement>('.action-priority-bar');
