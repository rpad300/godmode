...
function showNoProjectState(): void {
  ...
  dashboardContainer.innerHTML = `
    <div class="dashboard gm-container">
      <div class="gm-empty-state gm-empty-state--no-project">
        <svg class="gm-empty-state__icon" width="96" height="96" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>
        </svg>
        <h2 class="gm-empty-state__title">No Project Selected</h2>
        <p class="gm-empty-state__text gm-prose">
          Select a project from the dropdown above, or create a new one to get started with your data management.
        </p>
        <button id="create-project-empty-cta" class="gm-btn gm-btn--primary gm-btn--lg">
...
async function refreshDashboardPanel(): Promise<void> {
  ...
  dashboardContainer.innerHTML = '<div class="loading-placeholder gm-loading">Loading dashboard...</div>';
...
function initializePanels(): void {
  ...
  if (filesContainer && filesContainer.querySelectorAll('.documents-panel-minimal').length === 0) {
    ...
    import('./components/DocumentsPanel').then(({ default: createDocumentsPanel }) => {
      ...
      filesContainer.innerHTML = '<div class="gm-inline-status gm-inline-status--error">Failed to load files panel</div>';
    });
  }
