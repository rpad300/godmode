...
async function calculateBillableCost(projectId, providerCostUsd, totalTokens) {
    ...
    const FALLBACK_USD_TO_EUR_RATE = 0.92; // deterministic fallback used by unit tests
    
    const client = getAdminClient() || getClient();
    if (!client) {
        // Fallback: no markup, just use deterministic fallback rate
        const providerEur = providerCostUsd * FALLBACK_USD_TO_EUR_RATE;
        return {
    ...
    try {
        ...
        // Deterministic fallback (no markup)
        const providerEur = providerCostUsd * FALLBACK_USD_TO_EUR_RATE;
        return {
            provider_cost_eur: providerEur,
            billable_cost_eur: providerEur,
