...
const { ensureNodeModulesPath } = require('../infra/nodeModuleResolution');

// Fix module resolution for project named 'node_modules'
ensureNodeModulesPath({ nodeModulesPath: path.join(__dirname, '..', '..', 'node_modules') });
...
class SupabaseStorage {
    ...
    /**
     * Set current project context (global mutable state).
     * Prefer using forProject(projectId) inside request handlers to avoid cross-request leakage.
     */
    ...
    /**
     * Create a request-scoped view of this storage bound to a specific projectId.
     * This avoids mutating global currentProjectId (important for multi-project segregation).
     */
    forProject(projectId) {
        const child = Object.create(this);
        child.currentProjectId = projectId;
        // isolate cache per request/project view
        child._cache = new Map();
        return child;
    }
    ...
    async createProject(name, userRole = '') {
        ...
        await this.supabase.from('project_config').insert({
            project_id: data.id,
            user_role: userRole || '',
            updated_by: user.id
        });
    ...
    /**
     * List all projects for current user (requires auth)
     */
    ...
    async addContact(contact) {
        ...
        const { data, error } = await this.supabase
            ...
            .insert({
                ...
                photo_url: contact.photo_url || contact.photoUrl || null,
    ...
    async getChatMessages(sessionId) {
        // Ensure session belongs to current project
