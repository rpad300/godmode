...
async function handleContacts(ctx) {
    ...
    function getRequestProjectId(parsedUrl) {
        const q = parsedUrl?.query || {};
        const pid = q.project_id || q.projectId || req.headers['x-project-id'];
        if (typeof pid === 'string' && isValidUUID(pid)) return pid;
        const current = storage.getCurrentProject?.();
        if (current?.id && isValidUUID(current.id)) return current.id;
        return null;
    }

    // GET /api/contacts - List contacts (request-scoped project)
    if (pathname === '/api/contacts' && req.method === 'GET') {
        try {
            const parsedUrl = parseUrl(req.url);
            const projectId = getRequestProjectId(parsedUrl);
            ...
            // Supabase mode: avoid storage cache; use request-scoped project view
            if (storage._supabase && projectId && typeof storage._supabase.forProject === 'function') {
                ...
                const contacts = await sb.getContacts(Object.keys(filter).length > 0 ? filter : null);

                // Enrich contacts with ALL team memberships (N:N)
                const allTeams = (await sb.getTeams()) || [];
                const enrichedContacts = (contacts || []).map(c => {
                    const memberTeams = allTeams.filter(t =>
                        t.members?.some(m => (m.contact?.id || m.contact_id || m.contactId) === c.id)
                    ).map(t => ({
                        id: t.id,
                        name: t.name,
    ...
    // GET /api/contacts/stats - Get contact statistics (request-scoped project)
    if (pathname === '/api/contacts/stats' && req.method === 'GET') {
        try {
            const projectId = (typeof req.headers['x-project-id'] === 'string' && isValidUUID(req.headers['x-project-id']))
                ? req.headers['x-project-id']
                : (storage.getCurrentProject?.()?.id || null);

            if (storage._supabase && projectId && typeof storage._supabase.forProject === 'function') {
                const sb = storage._supabase.forProject(projectId);
                const contacts = await sb.getContacts();
                const teams = await sb.getTeams();
                const totalContacts = (contacts || []).length;
                const totalTeams = (teams || []).length;
                jsonResponse(res, { ok: true, totalContacts, totalTeams });
                return true;
            }
    ...
    // GET /api/contacts/:id - Get single contact (request-scoped project)
    ...
    if (contactGetMatch && req.method === 'GET') {
        ...
        const projectId = (typeof headerPid === 'string' && isValidUUID(headerPid))
            ? headerPid
            : (storage.getCurrentProject?.()?.id || null);
        ...
        try {
            if (storage._supabase && projectId && typeof storage._supabase.forProject === 'function') {
                const sb = storage._supabase.forProject(projectId);
                const contact = await sb.getContactById(contactId);
                if (!contact) {
