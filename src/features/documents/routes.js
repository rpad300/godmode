...
async function handleDocuments(ctx) {
    ...
    if (pathname === '/api/documents' && req.method === 'GET') {
        ...
        try {
            // Project context must be request-scoped (multi-project segregation)
            // Prefer explicit project header sent by frontend; fall back to storage current project for legacy callers.
            const headerProjectId = req.headers['x-project-id'];
            const projectId = (typeof headerProjectId === 'string' && isValidUUID(headerProjectId))
                ? headerProjectId
                : storage._supabase.getProjectId();
    ...
    if (downloadMatch && req.method === 'GET') {
        ...
        try {
            ...
            // Google Drive-backed documents (filepath = gdrive:<fileId>)
            if (doc.filepath && typeof doc.filepath === 'string' && doc.filepath.startsWith('gdrive:')) {
                try {
                    const driveIntegration = require('../../integrations/googleDrive/drive');
                    const fileId = driveIntegration.parseDriveFileId(doc.filepath);

                    // Need projectId to resolve project-scoped secret
                    const { data: docProject } = await storage._supabase.supabase
                        .from('documents')
                        .select('project_id')
                        .eq('id', docId)
                        .single();

                    const projectId = docProject?.project_id;
                    if (!projectId) {
                        jsonResponse(res, { error: 'Document missing project_id' }, 500);
                        return true;
                    }

                    const driveClient = await driveIntegration.getDriveClientForProject(projectId);
                    if (!driveClient.success) {
                        jsonResponse(res, { error: driveClient.error }, 400);
                        return true;
                    }

                    const dl = await driveIntegration.downloadFile({ drive: driveClient.drive, fileId });
                    if (!dl.success) {
                    ...
                    dl.stream.on('error', (e) => {
                        console.error('[Download] Drive stream error:', e.message);
                        if (!res.headersSent) {
                    ...
                    console.error('[Download] Drive error:', e.message);
                    jsonResponse(res, { error: 'Download failed' }, 500);
                    return true;
                }
            }
    ...
    // GET /api/documents/:id/preview - Inline preview (best-effort)
    // For Drive-backed docs this proxies the content so users don't need Drive permissions.
    const previewMatch = pathname.match(/^\/api\/documents\/([a-f0-9\-]+)\/preview$/i);
        ...
        try {
            ...
            if (doc.filepath && typeof doc.filepath === 'string' && doc.filepath.startsWith('gdrive:')) {
                try {
                    ...
                    res.setHeader('X-Content-Type-Options', 'nosniff');
                    // Allow inline rendering but block any external loads
                    res.setHeader('Content-Security-Policy', "default-src 'none'; sandbox");

                    dl.stream.on('error', (e) => {
                        console.error('[Preview] Drive stream error:', e.message);
                        if (!res.headersSent) {
                            jsonResponse(res, { error: 'Preview failed' }, 500);
                        } else {
                            res.end();
                        }
                    });
                    ...
                    console.error('[Preview] Drive error:', e.message);
                    jsonResponse(res, { error: 'Preview failed' }, 500);
                    return true;
                }
            }
