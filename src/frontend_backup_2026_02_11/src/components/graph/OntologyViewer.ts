    ...
    ${entity.examples && entity.examples.length > 0 ? `
      <div class="detail-section">
        ...
        <ul class="examples-list">
          ${entity.examples.map((ex: string) => `<li>${escapeHtml(String(ex))}</li>`).join('')}
        </ul>
      </div>
    ` : ''}
  `;
...
function renderGraphTab(
      ...
      <div class="ontology-legend-items">
        ${Object.entries(state.schema.entityTypes).map(([name, entity]) => `
          <span class="graph-legend-item">
            <span class="graph-legend-dot" style="--legend-color: ${(entity as unknown as Record<string, unknown>).color || '#6366f1'}"></span>
            ${escapeHtml(name)}
          </span>
        `).join('')}
      </div>
  ...
  setTimeout(() => {
    ...
    Object.entries(state.schema!.entityTypes).forEach(([name, entity]) => {
      const color = (entity as unknown as Record<string, unknown>).color as string || '#6366f1';
    ...
    Object.entries(state.schema!.relationTypes).forEach(([name, relation]) => {
      const rel = relation as unknown as Record<string, unknown>;
