...
import { formatRelativeTime, formatFileSize, formatDateTime } from '@lib/format';
...
async function loadContentPreview(container: HTMLElement, docId: string): Promise<void> {
  ...
  try {
    ...
    const response = await http.get<DocumentContentResponse>(`/api/documents/${docId}`);
      ...
      if (runAnalysisBtn) {
        runAnalysisBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            const response = await http.post<{ success: boolean; error?: string }>(`/api/documents/${docId}/reprocess`, {});
