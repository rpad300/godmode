import{c as j,o as y,t as r,H as p}from"./main-v_cFye9p.js";import"./modulepreload-polyfill-B5Qt9EMX.js";function W(e={}){const s={schema:null,suggestions:[],typeStats:null,syncStatus:null,changes:[],workerStatus:null,workerStats:null,jobs:[],jobLog:[],compliance:null,falkorBrowserInfo:null,unusedTypes:null,activeTab:"entities",searchQuery:"",isLoading:!0,isAnalyzing:!1,isSyncing:!1},i=j("div",{className:"ontology-viewer"});return i.innerHTML=`
  ...
  `,A(i,s,e),i}async function A(e,s,i){e.querySelectorAll(".ontology-tab").forEach(c=>{y(c,"click",()=>{const l=c.getAttribute("data-tab");l&&B(e,s,l,i)})});const n=e.querySelector("#ontology-search");n&&y(n,"input",()=>{s.searchQuery=n.value.toLowerCase(),L(e,s,i)});const o=e.querySelector("#btn-sync");o&&y(o,"click",async()=>{if(s.isSyncing)return;s.isSyncing=!0,o.classList.add("syncing"),r.info("Syncing ontology to graph...");const c=await p.forceOntologySync();s.isSyncing=!1,o.classList.remove("syncing"),c.ok?(r.success("Ontology synced to graph"),await T(e,s,i)):r.error(`Sync failed: ${c.error}`)}),await T(e,s,i)}async function T(e,s,i){try{const[t,n,o,c,l,a,g,h,v]=await Promise.all([p.getOntologySchema(),p.getOntologySuggestions(),p.getOntologyTypeStats(),p.getOntologySyncStatus(),p.getOntologyChanges({limit:20}),p.getBackgroundWorkerStatus(),p.getOntologyJobs(),p.getBackgroundWorkerLog({limit:10}),p.getFalkorDBBrowserInfo()]);s.schema=t,s.suggestions=n.filter(w=>w.status==="pending"),s.typeStats=o,s.syncStatus=c,s.changes=l,s.workerStatus=a?.status||null,s.workerStats=a?.stats||null,s.jobs=g,s.jobLog=h,s.falkorBrowserInfo=v,s.isLoading=!1;const b=e.querySelector("#suggestions-badge");b&&(s.suggestions.length>0?(b.textContent=String(s.suggestions.length),b.classList.remove("hidden")):b.classList.add("hidden")),V(e,s),O(e,s),U(e,s),L(e,s,i)}catch(t){console.error("[OntologyViewer] Failed to load:",t);const n=e.querySelector("#ontology-content");n&&(n.innerHTML=`
        <div class="error-state">
          <p>Failed to load ontology schema</p>
          <button class="btn btn-sm" onclick="location.reload()">Retry</button>
        </div>
      `)}}function O(e,s){const i=e.querySelector("#sync-indicator");if(!i)return;const t=i.querySelector(".sync-dot");if(!t)return;s.syncStatus?s.syncStatus.syncInProgress?(t.className="sync-dot syncing",i.title="Syncing..."):s.syncStatus.isListening&&s.syncStatus.graphConnected?(t.className="sync-dot connected",i.title=`Connected - Last sync: ${s.syncStatus.lastSyncAt?new Date(s.syncStatus.lastSyncAt).toLocaleTimeString():"never"}`):s.syncStatus.graphConnected?(t.className="sync-dot partial",i.title="Connected (realtime sync disabled)"):(t.className="sync-dot disconnected",i.title="Graph not connected"):(t.className="sync-dot unknown",i.title="Status unknown");const n=e.querySelector("#ontology-source");n&&s.syncStatus?.ontologySource&&(n.textContent=`Source: ${s.syncStatus.ontologySource}`)}function B(e,s,i,t){s.activeTab=i,e.querySelectorAll(".ontology-tab").forEach(o=>{o.classList.toggle("active",o.getAttribute("data-tab")===i)}),L(e,s,t)}function L(e,s,i){const t=e.querySelector("#ontology-content");if(!(!t||!s.schema))switch(s.activeTab){case"entities":M(t,s,i);break;case"relations":q(t,s,i);break;case"graph":R(t,s);break;case"analytics":I(t,s,e);break;case"suggestions":x(t,s,e);break;case"history":N(t,s);break;case"jobs":S(t,s);break;case"tools":$(t,s);break}}function M(e,s,i){if(!s.schema||!s.schema.entityTypes){e.innerHTML='<div class="empty-state"><p>No entity types defined in schema</p></div>';return}const t=Object.entries(s.schema.entityTypes).filter(([n,o])=>s.searchQuery?n.toLowerCase().includes(s.searchQuery)||(o.label||"").toLowerCase().includes(s.searchQuery)||(o.description||"").toLowerCase().includes(s.searchQuery):!0).sort((n,o)=>n[0].localeCompare(o[0]));if(t.length===0){e.innerHTML=`
    ...
    <div class="ontology-layout">
      <div class="entity-list">
        ${t.map(([n,o])=>{const c=Object.entries(o.properties||{}),l=c.length>5;return`
            <div class="entity-card" data-entity="${n}">
              <div class="entity-header">
                <div class="entity-icon-wrapper" style="--entity-color: ${o.color||"#6366f1"}">
              ...
              <div class="entity-properties">
                <h5>PROPERTIES (${c.length})</h5>
                <div class="properties-list" data-entity="${n}" data-expanded="false">
                  ${c.slice(0,5).map(([a,g])=>`
                    <div class="property-row">
                      <span class="property-name ${g.required?"required":""}">${u(a)}</span>
                      <span class="property-type">${g.type||"string"}</span>
                    </div>
                  `).join("")}
                  ${l?`
                    <button class="btn-expand-props" data-entity="${n}">+${c.length-5} more</button>
  ...
  `,e.querySelectorAll(".entity-card").forEach(n=>{y(n,"click",o=>{if(o.target.classList.contains("btn-expand-props"))return;const c=n.getAttribute("data-entity");if(c&&s.schema?.entityTypes[c]){e.querySelectorAll(".entity-card").forEach(g=>g.classList.remove("selected")),n.classList.add("selected"),E(e,c,s.schema.entityTypes[c]);const l=e.querySelector(".entity-list"),a=e.querySelector("#entity-detail-panel");if(l&&a){const g=l.getBoundingClientRect(),v=n.getBoundingClientRect().top-g.top;a.style.marginTop=`${Math.max(0,v)}px`}i.onEntitySelect?.(s.schema.entityTypes[c])}})}),e.querySelectorAll(".btn-expand-props").forEach(n=>{y(n,"click",o=>{o.stopPropagation();const c=n.getAttribute("data-entity"),l=e.querySelector(`.properties-list[data-entity="${c}"]`),a=l?.querySelector(".properties-hidden");a&&(l?.getAttribute("data-expanded")==="true"?(a.classList.add("hidden"),n.textContent=`+${a.querySelectorAll(".property-row").length} more`,l?.setAttribute("data-expanded","false")):(a.classList.remove("hidden"),n.textContent="Show less",l?.setAttribute("data-expanded","true")))})})}function E(e,s,i){const t=e.querySelector("#entity-detail-panel");if(!t)return;const n=Object.entries(i.properties||{});t.innerHTML=`
    <div class="detail-header" style="--entity-color: ${i.color||"#6366f1"};">
      <h3>${u(i.label||s)}</h3>
        ...
        <div class="relation-card" data-relation="${n}">
          ...
          <div class="relation-types">
            <div class="from-types">
              ...
              ${(o.fromTypes||[]).map(c=>`<span class="type-chip">${u(c)}</span>`).join("")}
            </div>
            <div class="to-types">
              <span class="types-label">To:</span>
              ${(o.toTypes||[]).map(c=>`<span class="type-chip">${u(c)}</span>`).join("")}
            </div>
          </div>
        </div>
      `).join("")}
    </div>
  `,e.querySelectorAll(".relation-card").forEach(n=>{y(n,"click",()=>{const o=n.getAttribute("data-relation");o&&s.schema?.relationTypes[o]&&i.onRelationSelect?.(s.schema.relationTypes[o])})})}function R(e,s,i){if(!s.schema?.entityTypes||!s.schema?.relationTypes){e.innerHTML='<div class="empty-state"><p>No schema available for graph visualization</p></div>';return}e.innerHTML=`
      ...
      <div class="ontology-legend-items">
        ${Object.entries(s.schema.entityTypes).map(([t,n])=>`
          <span class="graph-legend-item">
            <span class="graph-legend-dot" style="--legend-color: ${n.color||"#6366f1"}"></span>
            ${u(t)}
          </span>
        `).join("")}
      </div>
    </div>
  `,setTimeout(()=>{const t=e.querySelector("#ontology-graph-canvas");if(!t)return;const n=window.vis;if(!n){t.innerHTML='<p class="ontology-graph-fallback">vis.js not loaded</p>';return}const o=[];Object.entries(s.schema.entityTypes).forEach(([h,v])=>{const b=v.color||"#6366f1";o.push({id:h,label:h,color:b,shape:"dot",font:{color:H()}})});const c=[];Object.entries(s.schema.relationTypes).forEach(([h,v])=>{const b=v,w=b.fromTypes||[],d=b.toTypes||[];w.forEach(m=>{d.forEach(f=>{s.schema.entityTypes[m]&&s.schema.entityTypes[f]&&c.push({from:m,to:f,label:h,arrows:"to",color:{color:"rgba(156, 163, 175, 0.6)"}})})})});const l={nodes:new n.DataSet(o),edges:new n.DataSet(c)},a={nodes:{size:30,borderWidth:3,font:{size:14,face:"Inter, system-ui, sans-serif",strokeWidth:3,strokeColor:"rgba(255,255,255,0.8)"},shadow:{enabled:!0,size:10,color:"rgba(0,0,0,0.2)"}},edges:{font:{size:9,align:"top",color:"rgba(100,100,100,0.7)"},smooth:{type:"curvedCW",roundness:.2},width:1.5,selectionWidth:3,hoverWidth:2},physics:{enabled:!0,solver:"barnesHut",barnesHut:{gravitationalConstant:-3e3,centralGravity:.1,springLength:250,springConstant:.02,damping:.3,avoidOverlap:1},stabilization:{enabled:!0,iterations:200,fit:!0}},interaction:{hover:!0,tooltipDelay:200,zoomView:!0,dragView:!0},layout:{improvedLayout:!0,randomSeed:42}},g=new n.Network(t,l,a);g.once("stabilizationIterationsDone",()=>{g.fit()})},100)}function H(){return document.documentElement.getAttribute("data-theme")==="dark"?"#e5e7eb":"#1f2937"}function x(e,s,i){if(s.suggestions.length===0){const t=s.typeStats?.notInOntology?.entities?.length||s.typeStats?.notInOntology?.relations?.length,n=s.typeStats?.unused?.entities?.length||s.typeStats?.unused?.relations?.length;e.innerHTML=`
    ...
    `;const o=e.querySelector("#btn-run-ai-analysis");o&&y(o,"click",async()=>{r.info("Running AI analysis..."),o.classList.add("loading"),o.disabled=!0;const l=await p.runLLMAnalysis();if(o.classList.remove("loading"),o.disabled=!1,l){r.success(`Analysis complete: ${l.suggestions?.length||0} suggestions`);const a=await p.getOntologySuggestions();s.suggestions=a.filter(g=>g.status==="pending"),x(e,s,i),k(i,s)}else r.error("Analysis failed")});const c=e.querySelector("#btn-check-gaps");c&&y(c,"click",async()=>{r.info("Checking for gaps..."),c.classList.add("loading"),c.disabled=!0;const l=await p.triggerBackgroundAnalysis("gaps");if(c.classList.remove("loading"),c.disabled=!1,l){r.success("Gap check complete");const a=await p.getOntologySuggestions();s.suggestions=a.filter(g=>g.status==="pending"),x(e,s,i),k(i,s)}else r.error("Gap check failed")});return}e.innerHTML=`
  ...
  `,e.querySelectorAll(".btn-approve").forEach(t=>{y(t,"click",async n=>{n.stopPropagation();const o=t.getAttribute("data-id");o&&(await p.approveOntologySuggestion(o)?(r.success("Suggestion approved"),s.suggestions=s.suggestions.filter(l=>l.id!==o),x(e,s,i),k(i,s)):r.error("Failed to approve suggestion"))})}),e.querySelectorAll(".btn-reject").forEach(t=>{y(t,"click",async n=>{n.stopPropagation();const o=t.getAttribute("data-id");o&&(await p.rejectOntologySuggestion(o)?(r.info("Suggestion rejected"),s.suggestions=s.suggestions.filter(l=>l.id!==o),x(e,s,i),k(i,s)):r.error("Failed to reject suggestion"))})})}function I(e,s,i){const t=s.typeStats,n=t?.compliance;e.innerHTML=`
  ...
  `;const o=e.querySelector("#btn-analyze");o&&y(o,"click",async()=>{if(s.isAnalyzing)return;s.isAnalyzing=!0,o.classList.add("loading"),o.disabled=!0,r.info("Running LLM analysis... This may take a moment.");const l=await p.runLLMAnalysis();if(s.isAnalyzing=!1,o.classList.remove("loading"),o.disabled=!1,l){r.success(`Analysis complete: ${l.suggestions?.length||0} new suggestions`);const a=e.querySelector("#analysis-results");a&&(a.classList.remove("hidden"),a.innerHTML=`
          ...
          `);const g=await p.getOntologySuggestions();s.suggestions=g.filter(h=>h.status==="pending"),k(i,s)}else r.error("Analysis failed. Check console for details.")});const c=e.querySelector("#btn-auto-approve");c&&y(c,"click",async()=>{if(!s.suggestions.length){r.info("No pending suggestions to approve");return}const l=await p.autoApproveHighConfidence(.85);if(l.approved>0){r.success(`Auto-approved ${l.approved} high-confidence suggestions`);const a=await p.getOntologySuggestions();s.suggestions=a.filter(g=>g.status==="pending"),k(i,s)}else r.info("No suggestions met the confidence threshold")})}function N(e,s){if(!s.changes||s.changes.length===0){e.innerHTML=`
      <div class="empty-state">
      ...
      <div class="history-list">
        ${s.changes.map(i=>{const t=z(i.change_type),n=D(i.change_type);return`
            <div class="history-item">
              <div class="history-icon" style="--history-color: ${n}">
                ${t}
              </div>
              <div class="history-content">
                <div class="history-header">
                  <span class="history-type" style="--history-color: ${n}">${F(i.change_type)}</span>
  ...
  `,e.querySelectorAll(".btn-toggle-job").forEach(a=>{y(a,"click",async()=>{const g=a.getAttribute("data-job-id"),h=a.getAttribute("data-enabled")==="true";if(g){const v=await p.toggleOntologyJob(g,!h);v?(r.success(`Job ${v.enabled?"enabled":"disabled"}`),s.jobs=await p.getOntologyJobs(),S(e,s)):r.error("Failed to toggle job")}})}),e.querySelectorAll(".btn-run-job").forEach(a=>{y(a,"click",async()=>{const g=a.getAttribute("data-job-type");if(g){r.info(`Running ${g}...`);const h=await p.triggerBackgroundAnalysis(g);h&&h.status==="completed"?r.success(`${g} completed`):h?.error&&r.error(`Error: ${h.error}`),s.jobLog=await p.getBackgroundWorkerLog({limit:10}),S(e,s)}})}),[{id:"btn-run-full",type:"full"},{id:"btn-run-inference",type:"inference"},{id:"btn-run-dedup",type:"dedup"},{id:"btn-run-gaps",type:"gaps"}].forEach(({id:a,type:g})=>{const h=e.querySelector(`#${a}`);h&&y(h,"click",async()=>{r.info(`Running ${g} analysis...`),h.classList.add("loading"),h.disabled=!0;const v=await p.triggerBackgroundAnalysis(g);h.classList.remove("loading"),h.disabled=!1,v&&v.status==="completed"?r.success(`${g} analysis completed`):v?.error&&r.error(`Error: ${v.error}`);const b=await p.getBackgroundWorkerStatus();s.workerStatus=b?.status||null,s.workerStats=b?.stats||null,s.jobLog=await p.getBackgroundWorkerLog({limit:10}),S(e,s)})})}function $(e,s,i){const{compliance:t}=s;e.innerHTML=`
  ...
  `;const n=e.querySelector("#btn-validate-compliance")||e.querySelector("#btn-recheck-compliance");n&&y(n,"click",async()=>{r.info("Validating compliance..."),n.classList.add("loading"),n.disabled=!0;const d=await p.validateOntologyCompliance();n.classList.remove("loading"),n.disabled=!1,d?(s.compliance=d,r.success(`Compliance: ${d.score}% - ${d.issues.length} issues found`),$(e,s)):r.error("Failed to validate compliance")});const o=e.querySelector("#btn-clear-compliance");o&&y(o,"click",()=>{s.compliance=null,$(e,s)});const c=e.querySelector("#btn-extract-ontology");c&&y(c,"click",async()=>{r.info("Extracting ontology from graph..."),c.classList.add("loading");const d=await p.extractOntologyFromGraph();if(c.classList.remove("loading"),d.ok&&d.ontology){const m=Object.keys(d.ontology.entityTypes).length,f=Object.keys(d.ontology.relationTypes).length;r.success(`Extracted: ${m} entities, ${f} relations`)}else r.error(d.error||"Failed to extract ontology")});const l=e.querySelector("#btn-diff-ontology");l&&y(l,"click",async()=>{r.info("Comparing ontology with graph..."),l.classList.add("loading");const d=await p.getOntologyDiff();if(l.classList.remove("loading"),d){const{diff:m}=d,f=m.entitiesOnlyInA.length+m.relationsOnlyInA.length,C=m.entitiesOnlyInB.length+m.relationsOnlyInB.length;f===0&&C===0?r.success("Schema and graph are in sync!"):r.info(`Diff: ${f} only in schema, ${C} only in graph`)}else r.error("Failed to compare ontology")});const a=e.querySelector("#btn-merge-ontology");a&&y(a,"click",async()=>{r.info("Merging ontology..."),a.classList.add("loading");const d=await p.mergeOntology({mergeProperties:!0,mergeEndpoints:!0,save:!0});if(a.classList.remove("loading"),d.ok){const m=d.changes?.length||0;m>0?r.success(`Merged: ${m} changes applied`):r.info("No new types to merge")}else r.error("Failed to merge ontology")});const g=e.querySelector("#btn-find-unused");g&&y(g,"click",async()=>{r.info("Finding unused types..."),g.classList.add("loading");const d=await p.findUnusedOntologyTypes();g.classList.remove("loading"),s.unusedTypes=d,d.entities.length+d.relations.length===0?r.success("All types are in use!"):r.info(`Found ${d.entities.length} unused entities, ${d.relations.length} unused relations`),$(e,s)});const h=e.querySelector("#btn-remove-unused");h&&y(h,"click",async()=>{if(!s.unusedTypes)return;r.info("Removing unused types..."),h.classList.add("loading"),h.disabled=!0;const d=await p.cleanupOntology({discardEntitiesWithoutRelations:!0,discardRelationsWithoutEntities:!0,save:!0});if(h.classList.remove("loading"),h.disabled=!1,d.ok){const m=(d.discardedEntities?.length||0)+(d.discardedRelations?.length||0);r.success(`Removed ${m} unused types`),s.unusedTypes=null,$(e,s)}else r.error("Failed to remove unused types")});const v=e.querySelector("#btn-clear-unused");v&&y(v,"click",()=>{s.unusedTypes=null,$(e,s)});const b=e.querySelector("#btn-cleanup-entities");b&&y(b,"click",async()=>{const d=await p.cleanupOntology({discardEntitiesWithoutRelations:!0,save:!0});d.ok&&r.success(`Removed ${d.discardedEntities?.length||0} orphan entities`)});const w=e.querySelector("#btn-cleanup-relations");w&&y(w,"click",async()=>{const d=await p.cleanupOntology({discardRelationsWithoutEntities:!0,save:!0});d.ok&&r.success(`Removed ${d.discardedRelations?.length||0} orphan relations`)})}function G(e){return e.replace("ontology_","").replace(/_/g," ").replace(/\b\w/g,s=>s.toUpperCase())}function V(e,s){const i=e.querySelector("#jobs-badge");i&&(s.workerStatus?.isRunning?(i.textContent="",i.className="badge job-status-badge running",i.classList.remove("hidden"),i.title="Job running"):s.workerStatus?.hasPendingAnalysis?(i.textContent="",i.className="badge job-status-badge pending",i.classList.remove("hidden"),i.title="Analysis pending"):i.classList.add("hidden"))}function U(e,s){const i=e.querySelector("#ontology-stats"),t=e.querySelector("#ontology-version");if(i&&s.schema){const n=Object.keys(s.schema.entityTypes||{}).length,o=Object.keys(s.schema.relationTypes||{}).length,c=Object.keys(s.schema.queryPatterns||{}).length;i.innerHTML=`
      <span class="stat">${n} entities</span>
      <span class="stat">${o} relations</span>
      <span class="stat">${c} patterns</span>
    `}t&&s.schema&&(t.innerHTML=`Schema v${s.schema.version}`)}function k(e,s){const i=e.querySelector("#suggestions-badge");i&&(s.suggestions.length>0?(i.textContent=String(s.suggestions.length),i.classList.remove("hidden")):i.classList.add("hidden"))}function u(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}export{W as createOntologyViewer};
//# sourceMappingURL=OntologyViewer-DZa4q9dC.js.map
