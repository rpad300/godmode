import{n as N,f as S,h as b,t as y,m as B,p as D}from"./main-v_cFye9p.js";import"./modulepreload-polyfill-B5Qt9EMX.js";let L=!1,C=null;function ve(n){const{document:e}=n,t=document.createElement("div");t.className="document-preview-modal",t.innerHTML=`
    <style>
  ...
  `,t.querySelectorAll(".preview-tab").forEach(r=>{r.addEventListener("click",()=>{const c=r.getAttribute("data-tab");H(t,c)})}),t.querySelector("#favorite-btn")?.addEventListener("click",()=>ee(t,e)),t.querySelector("#share-btn")?.addEventListener("click",()=>H(t,"share")),t.querySelector("#download-btn")?.addEventListener("click",()=>te(e)),t.querySelector("#reprocess-btn")?.addEventListener("click",()=>ie(e,n.onUpdate)),t.querySelector("#upload-version-btn")?.addEventListener("click",()=>ae(e,t,n.onUpdate)),t.querySelector("#generate-link-btn")?.addEventListener("click",()=>se(t,e)),t.querySelector("#copy-link-btn")?.addEventListener("click",()=>oe(t)),V(t,e.id),M(t,e.id),q(t,e.id),P(t,e.id),F(t,e.id),Y(t,e.id),Z(e.id).then(r=>{L=r,R(t)});const a=document.createElement("div");a.className="modal-overlay document-preview-overlay";const o=document.createElement("div");o.className="modal-container document-preview-modal-container";const i=document.createElement("button");i.className="document-preview-close-btn",i.innerHTML="×",i.onclick=()=>z(a,n.onClose),t.classList.add("position-relative"),t.appendChild(i),o.appendChild(t),a.appendChild(o),a.addEventListener("click",r=>{r.target===a&&z(a,n.onClose)});const l=r=>{r.key==="Escape"&&(z(a,n.onClose),document.removeEventListener("keydown",l))};document.addEventListener("keydown",l),document.body.appendChild(a)}function z(n,e){C&&(clearInterval(C),C=null),n.style.animation="fadeOut 0.2s ease-out",setTimeout(()=>{n.remove(),e?.()},200)}function H(n,e){n.querySelectorAll(".preview-tab").forEach(t=>{t.classList.toggle("active",t.getAttribute("data-tab")===e)}),n.querySelectorAll(".preview-section").forEach(t=>{t.classList.toggle("active",t.getAttribute("data-section")===e)})}async function V(n,e){const t=n.querySelector("#content-preview");try{const o=(await b.get(`/api/documents/${e}`)).data,i=o.document?.content||o.content;if(i){const l=i.length>5e3;t.innerHTML=`<pre class="conv-notes-pre">${d(i.substring(0,5e3))}${l?`

... (truncated)`:""}</pre>`}else t.innerHTML='<div class="content-empty">No content extracted</div>'}catch(a){console.error("[Preview] Failed to load content:",a),t.innerHTML='<div class="content-empty">Failed to load content</div>'}}async function M(n,e){const t=n.querySelector("#entities-full-list"),a=n.querySelector("#entities-tab"),o=n.querySelector("#entities-count"),i=n.querySelector("#people-count");try{const r=(await b.get(`/api/documents/${e}/extraction`)).data.extraction;if(!r){t.innerHTML='<div class="empty-section">No extraction data available</div>';return}const c=r.entities||[],m=r.facts||[],u=r.decisions||[],g=r.action_items||[],h=r.questions||[],s=r.risks||[],x=r.participants||[],$=c.filter(p=>p.type?.toLowerCase()==="person"),k=[...x];for(const p of $)k.some(w=>w.name?.toLowerCase()===p.name?.toLowerCase())||k.push({name:p.name,role:void 0,organization:void 0});i&&(i.textContent=String(k.length));const f=c.length+m.length+u.length+g.length+h.length+s.length+k.length;if(f===0){t.innerHTML='<div class="empty-section">No entities extracted</div>';return}a&&a.classList.remove("hidden"),o&&(o.textContent=String(f));let v=`<style>
      ...
      .entity-item-meta {
        ...
        `).join("")}
      </div>`),t.innerHTML=v,O(n,e)}catch{t.innerHTML='<div class="empty-section">Failed to load entities</div>'}}function O(n,e){const t=n.querySelector("#people-section");if(!t)return;const a=window.currentProjectId||document.body.dataset.projectId||n.closest("[data-project-id]")?.getAttribute("data-project-id")||"";t.querySelectorAll(".person-link-btn").forEach(o=>{o.addEventListener("click",async i=>{i.stopPropagation();const l=o.dataset.name||"";await W(o,l,a,n,e)})}),t.querySelectorAll(".person-create-btn").forEach(o=>{o.addEventListener("click",async i=>{i.stopPropagation();const l=o.dataset.name||"",r=o.dataset.role||"",c=o.dataset.org||"";await G(l,r,c,a,o,n,e)})})}async function W(n,e,t,a,o){document.querySelectorAll(".contact-link-dropdown").forEach(u=>u.remove());const i=document.createElement("div");i.className="contact-link-dropdown",i.innerHTML=`
    <style>
  ...
  `;const l=n.getBoundingClientRect();i.style.position="fixed",i.style.top=`${l.bottom+4}px`,i.style.left=`${Math.max(10,l.left-100)}px`,document.body.appendChild(i);const r=i.querySelector("input");r?.focus();try{const g=(await b.get(`/api/contacts?project_id=${t}&limit=100`)).data.contacts||[],h=i.querySelector(".contact-link-list");g.length===0?h.innerHTML='<div class="contact-link-empty">No contacts found</div>':A(h,g,e,t,a,o,i),r?.addEventListener("input",()=>{const s=r.value.toLowerCase(),x=g.filter($=>$.name?.toLowerCase().includes(s)||$.email?.toLowerCase().includes(s));A(h,x,e,t,a,o,i)})}catch(u){console.error("[People] Failed to load contacts:",u);const g=i.querySelector(".contact-link-list");g.innerHTML='<div class="contact-link-empty">Failed to load contacts</div>'}const c=u=>{!i.contains(u.target)&&u.target!==n&&(i.remove(),document.removeEventListener("click",c))};setTimeout(()=>document.addEventListener("click",c),100);const m=u=>{u.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",m))};document.addEventListener("keydown",m)}function A(n,e,t,a,o,i,l){if(e.length===0){n.innerHTML='<div class="contact-link-empty">No matching contacts</div>';return}n.innerHTML=e.map(r=>{const c=(r.name||"?").split(" ").map(m=>m[0]).slice(0,2).join("").toUpperCase();return`
    ...
    `}).join(""),n.querySelectorAll(".contact-link-item").forEach(r=>{r.addEventListener("click",async()=>{const c=r.dataset.contactId||"",m=r.dataset.contactName||"";try{await b.post("/api/contacts/link-participant",{projectId:a,participantName:t,contactId:c}),y.success(`Linked "${t}" to ${m}`),l.remove(),M(o,i)}catch(u){console.error("[People] Failed to link:",u),y.error("Failed to link contact")}})})}async function G(n,e,t,a,o,i,l){const r=o.textContent;o.textContent="Creating...",o.disabled=!0;try{const c=await b.post("/api/contacts",{project_id:a,name:n,role:e||void 0,organization:t||void 0,source:"document_extraction"});c.data.ok&&c.data.contact?(y.success(`Created contact: ${c.data.contact.name}`),M(i,l)):(y.error(c.data.error||"Failed to create contact"),o.textContent=r,o.disabled=!1)}catch(c){console.error("[People] Failed to create contact:",c),y.error("Failed to create contact"),o.textContent=r,o.disabled=!1}}async function q(n,e){const t=n.querySelector("#analysis-list");try{const o=(await b.get(`/api/documents/${e}/analysis`)).data.analyses||[];if(n.querySelector("#analysis-count").textContent=String(o.length),o.length===0){t.innerHTML=`
      ...
      `;const i=t.querySelector("#run-analysis-btn");i&&i.addEventListener("click",async()=>{const l=i,r=l.textContent;l.disabled=!0,l.textContent="Analyzing...";try{const c=await b.post(`/api/documents/${e}/reprocess`,{});c.data.success?(y.success("Analysis started"),setTimeout(()=>q(n,e),3e3)):(y.error(c.data.error||"Failed to start analysis"),l.disabled=!1,l.textContent=r)}catch(c){y.error(c.message||"Failed to run analysis"),l.disabled=!1,l.textContent=r}});return}t.innerHTML=o.map((i,l)=>`
      <div class="analysis-item" data-id="${i.id}">
        <div class="analysis-version">v${o.length-l}</div>
    ...
    `).join(""),t.querySelectorAll(".view-analysis-btn").forEach((i,l)=>{i.addEventListener("click",()=>{const r=o[l];J(n,r)})}),t.querySelectorAll(".compare-analysis-btn").forEach((i,l)=>{i.addEventListener("click",()=>{if(o.length<2){y.info("Need at least 2 analyses to compare");return}y.info("Compare feature coming soon")})})}catch{t.innerHTML='<div class="empty-section">No analysis history available</div>'}}async function J(n,e){let t=e.result;if(e.document_id)try{const s=await b.get(`/api/documents/${e.document_id}/extraction`);s.data.extraction&&(t=s.data.extraction,console.log("[Analysis] Loaded enriched extraction from /extraction endpoint"))}catch(s){if(console.warn("[Analysis] Could not load extraction:",s),!t)try{const x=await b.get(`/api/documents/${e.document_id}`);t=x.data.document?.extraction_result||x.data.extraction_result}catch{}}const a=document.createElement("div");a.className="analysis-detail-overlay";const o=t?JSON.stringify(t,null,2):"No detailed result available";let i="",l=[];if(t){const s=t;s.summary?i=s.summary:s.meeting?.title&&(i=`Meeting: ${s.meeting.title}`);const x=s.participants||[],k=(s.entities||[]).filter(v=>v.type?.toLowerCase()==="person"),f=new Set;for(const v of x)v.name&&!f.has(v.name.toLowerCase())&&(f.add(v.name.toLowerCase()),l.push({name:v.name,role:v.role,organization:v.organization,contact_id:v.contact_id,contact_name:v.contact_name,contact_email:v.contact_email,contact_avatar:v.contact_avatar,contact_role:v.contact_role}));for(const v of k)v.name&&!f.has(v.name.toLowerCase())&&(f.add(v.name.toLowerCase()),l.push({name:v.name,contact_id:v.contact_id,contact_name:v.contact_name,contact_email:v.contact_email,contact_avatar:v.contact_avatar,contact_role:v.contact_role}))}const r=l.filter(s=>s.contact_id),c=l.filter(s=>!s.contact_id),m=l.length>0?`
    <div class="analysis-people-section">
      <h4 class="analysis-people-title">
        ...
        ${r.length>0?`<span class="analysis-people-badge analysis-people-badge-linked">✓ ${r.length} linked</span>`:""}
      </h4>
          ...
          <div class="analysis-people-chips">
            ${r.map(s=>`
              <div class="person-chip linked entity-chip entity-chip-linked" data-name="${d(s.name)}" data-contact-id="${d(s.contact_id||"")}" data-contact-name="${d(s.contact_name||"")}">
                ...
                `:`
                  <div class="entity-chip-avatar-placeholder">${(s.contact_name||s.name||"?").split(" ").map(x=>x[0]).slice(0,2).join("").toUpperCase()}</div>
                `}
          ...
          <div class="analysis-people-chips">
            ${c.map(s=>`
              <div class="person-chip unlinked entity-chip entity-chip-unlinked" data-name="${d(s.name)}" data-role="${d(s.role||"")}" data-org="${d(s.organization||"")}">
                <div class="entity-chip-avatar-placeholder">${(s.name||"?").split(" ").map(x=>x[0]).slice(0,2).join("").toUpperCase()}</div>
                <div class="entity-chip-body">
  ...
  `:"",u=document.createElement("div");u.className="analysis-detail-content",u.innerHTML=`
    ...
    <div class="analysis-detail-body">
      ...
      <details class="analysis-detail-raw-wrap">
        ...
        <pre class="analysis-detail-raw-pre">${d(o)}</pre>
      </details>
    </div>
  `,a.appendChild(u),document.body.appendChild(a),u.querySelector(".close-analysis-btn")?.addEventListener("click",()=>a.remove()),a.addEventListener("click",s=>{s.target===a&&a.remove()});const h=window.currentProjectId||document.body.dataset.projectId||n.closest("[data-project-id]")?.getAttribute("data-project-id")||"";u.querySelectorAll(".person-chip").forEach(s=>{const x=s;s.addEventListener("click",async $=>{$.stopPropagation();const k=x.dataset.name||"",f=x.dataset.role||"",v=x.dataset.org||"",p=x.dataset.contactId||"",w=x.dataset.contactName||"";await Q(x,k,f,v,h,p,w)})})}async function Q(n,e,t,a,o,i="",l=""){document.querySelectorAll(".person-link-menu").forEach(s=>s.remove());const r=!!i,c=document.createElement("div");c.className="person-link-menu",c.innerHTML=`
    <style>
      .person-link-menu {
        position: fixed;
        z-index: 10002;
  ...
  `;const m=n.getBoundingClientRect();c.style.position="fixed",c.style.top=`${Math.min(m.bottom+8,window.innerHeight-420)}px`,c.style.left=`${Math.min(m.left,window.innerWidth-320)}px`,document.body.appendChild(c);const u=c.querySelector("input");u?.focus();try{const x=(await b.get(`/api/contacts?project_id=${o}&limit=100`)).data.contacts||[],$=c.querySelector(".person-link-menu-list"),k=f=>{if(f.length===0){$.innerHTML='<div class="person-link-menu-empty">No matching contacts</div>';return}$.innerHTML=f.map(v=>{const p=(v.name||"?").split(" ").map(T=>T[0]).slice(0,2).join("").toUpperCase(),w=v.avatar_url||v.photo_url||"",_=v.role||"";return`
        ...
        `}).join(""),$.querySelectorAll(".person-link-menu-item").forEach(v=>{v.addEventListener("click",async()=>{const p=v.dataset.contactId||"",w=v.dataset.contactName||"",_=v.dataset.contactEmail||"",T=v.dataset.contactAvatar||"",I=v.dataset.contactRole||"";try{await b.post("/api/contacts/link-participant",{projectId:o,participantName:e,contactId:p}),y.success(`Linked "${e}" to ${w}`),c.remove(),j(n,e,p,w,_,T,I)}catch(U){console.error("[People] Failed to link:",U),y.error("Failed to link contact")}})})};k(x),u?.addEventListener("input",()=>{const f=u.value.toLowerCase(),v=x.filter(p=>p.name?.toLowerCase().includes(f)||p.email?.toLowerCase().includes(f));k(v)})}catch(s){console.error("[People] Failed to load contacts:",s);const x=c.querySelector(".person-link-menu-list");x.innerHTML='<div class="person-link-menu-empty">Failed to load contacts</div>'}c.querySelector(".cancel-btn")?.addEventListener("click",()=>c.remove()),c.querySelector(".unlink-btn")?.addEventListener("click",async()=>{try{await b.post("/api/contacts/unlink-participant",{projectId:o,participantName:e}),y.success(`Unlinked "${e}"`),c.remove(),K(n,e,t,a)}catch(s){console.error("[People] Failed to unlink:",s),y.error("Failed to unlink contact")}}),c.querySelector(".create-btn")?.addEventListener("click",async()=>{try{const s=await b.post("/api/contacts",{project_id:o,name:e,role:t||void 0,organization:a||void 0,source:"document_extraction"});s.data.ok&&s.data.contact?(y.success(`Created contact: ${s.data.contact.name}`),c.remove(),j(n,e,s.data.contact.id,s.data.contact.name,"",s.data.contact.avatar_url||"",s.data.contact.role||t)):y.error(s.data.error||"Failed to create contact")}catch(s){console.error("[People] Failed to create contact:",s),y.error("Failed to create contact")}});const g=s=>{!c.contains(s.target)&&s.target!==n&&(c.remove(),document.removeEventListener("click",g))};setTimeout(()=>document.addEventListener("click",g),100);const h=s=>{s.key==="Escape"&&(c.remove(),document.removeEventListener("keydown",h))};document.addEventListener("keydown",h)}function K(n,e,t,a){const o=(e||"?").split(" ").map(i=>i[0]).slice(0,2).join("").toUpperCase();n.classList.remove("linked"),n.classList.add("unlinked","entity-chip","entity-chip-unlinked"),delete n.dataset.contactId,delete n.dataset.contactName,n.innerHTML=`
  ...
  `}function j(n,e,t,a,o,i,l){const r=(a||e||"?").split(" ").map(c=>c[0]).slice(0,2).join("").toUpperCase();n.classList.remove("entity-chip-unlinked"),n.classList.add("linked","entity-chip","entity-chip-linked"),n.dataset.contactId=t,n.dataset.contactName=a,n.innerHTML=`
    ${i?`
  ...
  `}async function P(n,e){const t=n.querySelector("#versions-list");try{const o=(await b.get(`/api/documents/${e}/versions`)).data.versions||[];if(n.querySelector("#versions-count").textContent=String(o.length||1),o.length===0){t.innerHTML=`
    ...
    `).join("")}catch{t.innerHTML='<div class="empty-section">Version history not available</div>'}}async function F(n,e){const t=n.querySelector("#activity-timeline");try{const o=(await b.get(`/api/documents/${e}/activity`)).data.activities||[];if(o.length===0){t.innerHTML='<div class="empty-section">No activity recorded</div>';return}t.innerHTML=o.map(i=>`
      <div class="activity-item">
        <div class="activity-content">
          <div class="activity-avatar">
            ${i.user_avatar?`<img src="${i.user_avatar}" alt="">`:de(i.user_name||"System")}
          </div>
          <div class="activity-text">
            <strong>${d(i.user_name||"System")}</strong> ${ce(i.action)}
          </div>
          <div class="activity-time">${S(i.created_at)}</div>
        </div>
      </div>
    `).join("")}catch{t.innerHTML='<div class="empty-section">Activity not available</div>'}}async function Y(n,e){const t=n.querySelector("#notes-container"),a=n.querySelector("#notes-tab");try{const i=(await b.get(`/api/documents/${e}/extraction`)).data.extraction;if(!(i?.notes_rendered_text||(i?.notes?.key_points?.length||0)>0)){a&&a.classList.add("hidden");return}a&&a.classList.remove("hidden");const r=i?.notes_rendered_text||"",c=i?.notes;if(r)t.innerHTML=`
        <style>
          .notes-rendered {
            padding: 20px;
            background: var(--bg-secondary);
      ...
      `,n.querySelector("#copy-notes-btn")?.addEventListener("click",()=>{navigator.clipboard.writeText(r),y.success("Notes copied to clipboard")});else if(c){const m=c.key_points?.map(g=>`• ${g.text}`).join(`
...
`)||"";t.innerHTML=`
      ...
      `}}catch{t.innerHTML='<div class="empty-section">Meeting notes not available</div>',a&&a.classList.add("hidden")}}function d(n){const e=document.createElement("div");return e.textContent=n,e.innerHTML}function X(n){const e=n.split(`
`);let t="",a="",o="",i=!1,l=!1;const r=()=>{a&&o&&(t+=`
        <div class="notes-topic">
          <div class="notes-topic-title">${d(a)}</div>
          ${o}
        </div>
      `),a="",o=""};for(const c of e){const m=c.trim();if(m){if(!i&&m.startsWith("📝")){t+=`<div class="notes-header">${d(m)}</div>`,i=!0;continue}if(!l&&m.startsWith("🕞")){t+=`<div class="notes-meta">${d(m)}</div>`,l=!0;continue}if(m.startsWith("-")){const u=m.substring(1).trim();o+=`<div class="notes-bullet">${d(u)}</div>`;continue}r(),a=m}}return r(),t.includes("notes-topic")?t:`<pre class="conv-notes-pre">${d(n)}</pre>`}async function Z(n){try{return(await b.get(`/api/documents/${n}/favorite`)).data.is_favorite||!1}catch{return!1}}function R(n){const e=n.querySelector("#favorite-btn");e&&(e.classList.toggle("active",L),e.innerHTML=L?"★":"☆")}async function ee(n,e){try{await b.post(`/api/documents/${e.id}/favorite`),L=!L,R(n),y.success(L?"Added to favorites":"Removed from favorites")}catch{y.error("Failed to update favorite")}}function te(n){window.open(`/api/documents/${n.id}/download`,"_blank")}async function ne(n,e){try{const a=(await b.get(`/api/documents/${e}`)).data,o=["facts","decisions","risks","actions","questions"];for(const m of o){const u=n.querySelector(`[data-entity="${m}"]`);if(u){const g=u.querySelector(".entity-count");if(g){const h=a[`${m}_count`]||0;g.textContent=String(h),u.classList.add("updated"),setTimeout(()=>u.classList.remove("updated"),1500)}}}const i=n.querySelector("#summary-card"),l=n.querySelector("#doc-summary");i&&l&&(a.summary?(i.classList.remove("hidden"),l.textContent=a.summary):i.classList.add("hidden"));const r=n.querySelector("#content-preview");if(r&&a.content){const m=a.content.length>5e3;r.innerHTML=d(a.content.substring(0,5e3))+(m?`

... (truncated)`:"")}const c=n.querySelector(".status-chip");c&&(c.className=`status-chip ${a.status}`,c.textContent=E(a.status)),q(n,e),F(n,e),console.log("[Modal] Refreshed content for document:",e)}catch(t){console.error("[Modal] Failed to refresh content:",t)}}async function ie(n,e){const t=document.querySelector("#reprocess-btn"),a=document.querySelector(".document-preview-modal"),o=t?.innerHTML||"";try{t&&(t.disabled=!0,t.innerHTML='<span class="spinner"></span> Checking...');const l=(await b.get(`/api/documents/${n.id}/reprocess/check`)).data;if(!l.has_content){y.error("No content available for reprocessing");return}let r=!0;if(l.hash_match||l.total_entities>0){const g=l.hash_match?`Content has not changed since last processing.
...
- Use AI tokens`)}if(!r){y.info("Reprocessing cancelled");return}t&&(t.innerHTML='<span class="spinner"></span> Processing...'),y.info("Reprocessing started..."),await b.post(`/api/documents/${n.id}/reprocess`,{force:!0});let c=0;const m=60,u=async()=>{c++;try{const h=(await b.get(`/api/documents/${n.id}`)).data.status;return h==="processed"?(y.success("Document reprocessed successfully!"),a&&await ne(a,n.id),e?.(),!0):h==="failed"?(y.error("Reprocessing failed"),!0):c>=m?(y.warning("Reprocessing still in progress. Check back later."),!0):(t&&(t.innerHTML=`<span class="spinner"></span> Processing... (${c}s)`),!1)}catch{return c>=m}};C=setInterval(async()=>{await u()&&(C&&(clearInterval(C),C=null),t&&(t.disabled=!1,t.innerHTML=o))},2e3)}catch(i){console.error("[Reprocess] Error:",i),y.error("Failed to reprocess document")}finally{t&&(t.disabled=!1,t.innerHTML=o)}}async function ae(n,e,t){const a=document.createElement("input");a.type="file",a.accept=".pdf,.doc,.docx,.txt,.md",a.onchange=async()=>{const o=a.files?.[0];if(!o)return;const i=prompt("Change notes (optional):"),l=new FormData;l.append("file",o),i&&l.append("change_notes",i);try{y.info("Uploading new version..."),await B(`/api/documents/${n.id}/versions`,{method:"POST",body:l}),y.success("New version uploaded"),P(e,n.id),t?.()}catch{y.error("Failed to upload new version")}},a.click()}async function se(n,e){const t=n.querySelector("#share-expires").value,a=n.querySelector("#share-max-views").value,o=n.querySelector("#share-password").value,i=n.querySelector("#share-permissions").value;try{const l=await b.post(`/api/documents/${e.id}/share`,{expires:t,max_views:a?parseInt(a):null,password:o||null,permissions:[i==="download"?"view":i,...i==="download"?["download"]:[]]}),r=n.querySelector("#share-link-input");r.value=l.data.url,n.querySelector("#generate-link-btn")?.classList.add("hidden"),n.querySelector("#copy-link-btn")?.classList.remove("hidden"),y.success("Share link generated")}catch{y.error("Failed to generate share link")}}function oe(n){const e=n.querySelector("#share-link-input");navigator.clipboard.writeText(e.value),y.success("Link copied to clipboard")}function re(n){switch(n.split(".").pop()?.toLowerCase()){case"pdf":return"📄";case"doc":case"docx":return"📝";case"xls":case"xlsx":return"📊";case"ppt":case"pptx":return"📽️";case"txt":case"md":return"📃";case"jpg":case"jpeg":case"png":case"gif":return"🖼️";default:return"📁"}}function ce(n){return{created:"uploaded this document",viewed:"viewed this document",downloaded:"downloaded this document",updated:"updated this document",version_uploaded:"uploaded a new version",analyzed:"ran AI analysis",reprocessed:"reprocessed this document",shared:"shared this document",unshared:"removed share link",deleted:"deleted this document",restored:"restored this document",tagged:"added tags",favorited:"added to favorites",unfavorited:"removed from favorites",commented:"commented on this document"}[n]||n}function de(n){return n.split(" ").map(e=>e[0]).join("").substring(0,2).toUpperCase()}function E(n){return n.charAt(0).toUpperCase()+n.slice(1)}export{ve as showDocumentPreviewModal};
//# sourceMappingURL=DocumentPreviewModal-DEXaSRWs.js.map
