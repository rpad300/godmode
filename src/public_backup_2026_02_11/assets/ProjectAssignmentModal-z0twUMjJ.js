import{c as x,u as k,w as j,o as m,v as g,U as w,t as l,V as S,Z as M,h as $}from"./main-v_cFye9p.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const p="project-assignment-modal";let d=null,f=[];async function q(s){d=s;const{transcript:a}=s,i=document.querySelector(`[data-modal-id="${p}"]`);i&&i.remove();const t=x("div",{className:"project-assignment"});t.innerHTML=`
    <style>
      .project-assignment {
        padding: 16px 0;
      }
    ...
    <div class="assignment-info">
      <div class="assignment-title">${o(a.krisp_title||"Untitled Meeting")}</div>
      ${a.status_reason?`
  ...
  `;const e=k({id:p,title:"Assign to Project",content:t,size:"md"});document.body.appendChild(e),j(p),A(t),v(t,a),await z(t,a)}function A(s){const a=s.querySelector("#cancel-btn");a&&m(a,"click",()=>{g(p),d?.onClose?.()});const i=s.querySelector("#skip-btn");i&&m(i,"click",async()=>{if(!d?.transcript||!confirm("Are you sure you want to skip this transcript? It will not be processed."))return;await w(d.transcript.id,"Manually skipped")?(l.success("Transcript skipped"),g(p),d?.onAssign?.()):l.error("Failed to skip transcript")});const t=s.querySelector("#assign-btn");t&&m(t,"click",async()=>{s.querySelector(".candidate-item.selected, .project-select");let e=null;const n=s.querySelector(".candidate-item.selected");if(n)e=n.getAttribute("data-project-id");else{const r=s.querySelector(".project-select");r?.value&&(e=r.value)}if(!e||!d?.transcript){l.error("Please select a project");return}t.disabled=!0,t.textContent="Assigning...",await S(d.transcript.id,e)?(l.success("Transcript assigned to project"),g(p),d?.onAssign?.()):(l.error("Failed to assign project"),t.disabled=!1,t.textContent="Assign to Project")})}async function z(s,a){const i=s.querySelector("#assignment-content");if(i)try{f=(await $.get("/api/projects")).data.projects||[];const e=a.project_candidates||[];let n="";e.length>0&&(n+=`
            ...
            ${e.map(r=>`
              <div class="candidate-item" data-project-id="${r.projectId}">
                ...
                <div class="candidate-info">
                  <div class="candidate-name">${o(r.projectName||"Unknown")}</div>
                  <div class="candidate-code">${o(r.projectNumber||"")}</div>
                </div>
    ...
    `;e.topic?n+=`<div class="summary-topic">${o(e.topic)}</div>`:e.notes&&(n+=`<div class="summary-topic">${o(e.notes)}</div>`),e.keyPoints&&e.keyPoints.length>0&&(n+=`
        <div class="summary-section">
          <div class="summary-section-title">Key Points</div>
          <ul class="summary-list">
            ${e.keyPoints.slice(0,5).map(r=>`<li>${o(String(r))}</li>`).join("")}
          </ul>
        </div>
      `),e.actionItems&&e.actionItems.length>0&&(n+=`
        <div class="summary-section">
          <div class="summary-section-title">Action Items</div>
          <ul class="summary-list">
            ${e.actionItems.slice(0,5).map(r=>`<li>${o(String(r))}</li>`).join("")}
          </ul>
        </div>
      `),e.decisions&&e.decisions.length>0&&(n+=`
        <div class="summary-section">
          <div class="summary-section-title">Decisions</div>
          <ul class="summary-list">
            ${e.decisions.slice(0,3).map(r=>`<li>${o(String(r))}</li>`).join("")}
          </ul>
        </div>
      `),e.nextSteps&&(n+=`
        <div class="summary-section">
          <div class="summary-section-title">Next Steps</div>
          <p class="gm-text-sm gm-text-secondary gm-m-0">${o(e.nextSteps)}</p>
        </div>
      `);const c={krisp_metadata:"From Krisp",ai_generated:"AI Generated",excerpt_fallback:"Excerpt",no_content:""};e.source&&c[e.source]&&(n+=`<div class="summary-source">${c[e.source]}</div>`),t.innerHTML=n,b(s,a)}catch(e){console.error("[ProjectAssignmentModal] Summary error:",e),t.innerHTML=`
      <div class="summary-header">
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
        </svg>
    ...
    `,b(s,a)}}}function b(s,a){const i=s.querySelector(".refresh-summary-btn");i&&m(i,"click",async t=>{t.preventDefault(),t.stopPropagation(),await v(s,a,!0),l.success("Summary regenerated")})}function o(s){const a=document.createElement("div");return a.textContent=s,a.innerHTML}export{q as showProjectAssignmentModal};
//# sourceMappingURL=ProjectAssignmentModal-z0twUMjJ.js.map
