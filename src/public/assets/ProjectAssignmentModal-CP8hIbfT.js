import{d as x,r as k,u as j,o as m,q as g,E as w,t as l,F as S,J as M,h as $}from"./main-BEBbDhZA.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const p="project-assignment-modal";let d=null,f=[];async function P(s){d=s;const{transcript:a}=s,i=document.querySelector(`[data-modal-id="${p}"]`);i&&i.remove();const t=x("div",{className:"project-assignment"});t.innerHTML=`
    <style>
      .project-assignment {
        padding: 16px 0;
      }
  ...
  `;const e=k({id:p,title:"Assign to Project",content:t,size:"md"});document.body.appendChild(e),j(p),A(t),v(t,a),await q(t,a)}function A(s){const a=s.querySelector("#cancel-btn");a&&m(a,"click",()=>{g(p),d?.onClose?.()});const i=s.querySelector("#skip-btn");i&&m(i,"click",async()=>{if(!d?.transcript||!confirm("Are you sure you want to skip this transcript? It will not be processed."))return;await w(d.transcript.id,"Manually skipped")?(l.success("Transcript skipped"),g(p),d?.onAssign?.()):l.error("Failed to skip transcript")});const t=s.querySelector("#assign-btn");t&&m(t,"click",async()=>{s.querySelector(".candidate-item.selected, .project-select");let e=null;const n=s.querySelector(".candidate-item.selected");if(n)e=n.getAttribute("data-project-id");else{const r=s.querySelector(".project-select");r?.value&&(e=r.value)}if(!e||!d?.transcript){l.error("Please select a project");return}t.disabled=!0,t.textContent="Assigning...",await S(d.transcript.id,e)?(l.success("Transcript assigned to project"),g(p),d?.onAssign?.()):(l.error("Failed to assign project"),t.disabled=!1,t.textContent="Assign to Project")})}async function q(s,a){const i=s.querySelector("#assignment-content");if(i)try{f=(await $.get("/api/projects")).data.projects||[];const e=a.project_candidates||[];let n="";e.length>0&&(n+=`
    ...
    `,b(s,a)}}}function b(s,a){const i=s.querySelector(".refresh-summary-btn");i&&m(i,"click",async t=>{t.preventDefault(),t.stopPropagation(),await v(s,a,!0),l.success("Summary regenerated")})}function o(s){const a=document.createElement("div");return a.textContent=s,a.innerHTML}export{P as showProjectAssignmentModal};
//# sourceMappingURL=ProjectAssignmentModal-CP8hIbfT.js.map
