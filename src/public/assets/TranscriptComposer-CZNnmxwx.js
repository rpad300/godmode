import{c as y,o as a}from"./index-DRjFKNx1.js";let m="paste";function B(e={}){const r=document.getElementById("transcript-composer-overlay");r&&r.remove();const o=y("div",{id:"transcript-composer-overlay",className:"composer-overlay"}),s=S(e);o.appendChild(s),document.body.appendChild(o),a(o,"click",i=>{i.target===o&&(d(),e.onClose?.())}),requestAnimationFrame(()=>{o.classList.add("visible")})}function d(){const e=document.getElementById("transcript-composer-overlay");e&&(e.classList.remove("visible"),setTimeout(()=>e.remove(),200))}function S(e){const r=y("div",{className:"composer-modal"});r.innerHTML=`
  ...
  `,r.querySelectorAll(".composer-tab").forEach(s=>{a(s,"click",()=>{r.querySelectorAll(".composer-tab").forEach(i=>i.classList.remove("active")),s.classList.add("active"),m=s.getAttribute("data-mode"),q(r)})}),a(r.querySelector("#close-btn"),"click",()=>{d(),e.onClose?.()}),a(r.querySelector("#cancel-btn"),"click",()=>{d(),e.onClose?.()});const o=r.querySelector("#import-btn");return a(o,"click",()=>w(r,e)),h(r),r}function q(e){const r=e.querySelector("#composer-body");r.innerHTML=m==="paste"?g():C(),m==="paste"?h(e):T(e)}function g(){return`
...
Thanks for having us. Let's get started."></textarea>
  ...
  `;const u=t.querySelector("#processing-status"),v=t.querySelector("#processing-detail");try{const l=e.querySelector("#source-select"),n=new FormData,c=`transcript_${Date.now()}.txt`;n.append("file",new Blob([i],{type:"text/plain"}),c),n.append("folder","newtranscripts"),l?.value&&n.append("source",l.value),u.textContent="Uploading transcript...",v.textContent=`${k(i.length)} • ${c}`;const b=await fetch("/api/upload",{method:"POST",body:n,credentials:"include"});if(!b.ok){const z=await b.json().catch(()=>({}));throw new Error(z.error||`Upload failed (${b.status})`)}const f=await b.json();u.textContent="Transcript uploaded!",v.textContent="AI processing will start automatically",t.innerHTML=`
      <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
        <div style="
          width: 24px;
          height: 24px;
...
//# sourceMappingURL=TranscriptComposer-CZNnmxwx.js.map
