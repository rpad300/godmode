import{f as N,a as S,h as x,t as y,b as B,c as D}from"./main-BEBbDhZA.js";import"./modulepreload-polyfill-B5Qt9EMX.js";let L=!1,C=null;function ve(n){const{document:e}=n,t=document.createElement("div");t.className="document-preview-modal",t.innerHTML=`
    <style>
...
... (truncated)`:""}</pre>`}else t.innerHTML='<div class="content-empty">No content extracted</div>'}catch(a){console.error("[Preview] Failed to load content:",a),t.innerHTML='<div class="content-empty">Failed to load content</div>'}}async function M(n,e){const t=n.querySelector("#entities-full-list"),a=n.querySelector("#entities-tab"),o=n.querySelector("#entities-count"),i=n.querySelector("#people-count");try{const r=(await x.get(`/api/documents/${e}/extraction`)).data.extraction;if(!r){t.innerHTML='<div class="empty-section">No extraction data available</div>';return}const c=r.entities||[],m=r.facts||[],u=r.decisions||[],g=r.action_items||[],h=r.questions||[],s=r.risks||[],b=r.participants||[],$=c.filter(p=>p.type?.toLowerCase()==="person"),k=[...b];for(const p of $)k.some(w=>w.name?.toLowerCase()===p.name?.toLowerCase())||k.push({name:p.name,role:void 0,organization:void 0});i&&(i.textContent=String(k.length));const f=c.length+m.length+u.length+g.length+h.length+s.length+k.length;if(f===0){t.innerHTML='<div class="empty-section">No entities extracted</div>';return}a&&a.classList.remove("hidden"),o&&(o.textContent=String(f));let v=`<style>
  ...
  `;const l=n.getBoundingClientRect();i.style.position="fixed",i.style.top=`${l.bottom+4}px`,i.style.left=`${Math.max(10,l.left-100)}px`,document.body.appendChild(i);const r=i.querySelector("input");r?.focus();try{const g=(await x.get(`/api/contacts?project_id=${t}&limit=100`)).data.contacts||[],h=i.querySelector(".contact-link-list");g.length===0?h.innerHTML='<div class="contact-link-empty">No contacts found</div>':A(h,g,e,t,a,o,i),r?.addEventListener("input",()=>{const s=r.value.toLowerCase(),b=g.filter($=>$.name?.toLowerCase().includes(s)||$.email?.toLowerCase().includes(s));A(h,b,e,t,a,o,i)})}catch(u){console.error("[People] Failed to load contacts:",u);const g=i.querySelector(".contact-link-list");g.innerHTML='<div class="contact-link-empty">Failed to load contacts</div>'}const c=u=>{!i.contains(u.target)&&u.target!==n&&(i.remove(),document.removeEventListener("click",c))};setTimeout(()=>document.addEventListener("click",c),100);const m=u=>{u.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",m))};document.addEventListener("keydown",m)}function A(n,e,t,a,o,i,l){if(e.length===0){n.innerHTML='<div class="contact-link-empty">No matching contacts</div>';return}n.innerHTML=e.map(r=>{const c=(r.name||"?").split(" ").map(m=>m[0]).slice(0,2).join("").toUpperCase();return`
    ...
    `}).join(""),n.querySelectorAll(".contact-link-item").forEach(r=>{r.addEventListener("click",async()=>{const c=r.dataset.contactId||"",m=r.dataset.contactName||"";try{await x.post("/api/contacts/link-participant",{projectId:a,participantName:t,contactId:c}),y.success(`Linked "${t}" to ${m}`),l.remove(),M(o,i)}catch(u){console.error("[People] Failed to link:",u),y.error("Failed to link contact")}})})}async function G(n,e,t,a,o,i,l){const r=o.textContent;o.textContent="Creating...",o.disabled=!0;try{const c=await x.post("/api/contacts",{project_id:a,name:n,role:e||void 0,organization:t||void 0,source:"document_extraction"});c.data.ok&&c.data.contact?(y.success(`Created contact: ${c.data.contact.name}`),M(i,l)):(y.error(c.data.error||"Failed to create contact"),o.textContent=r,o.disabled=!1)}catch(c){console.error("[People] Failed to create contact:",c),y.error("Failed to create contact"),o.textContent=r,o.disabled=!1}}async function q(n,e){const t=n.querySelector("#analysis-list");try{const o=(await x.get(`/api/documents/${e}/analysis`)).data.analyses||[];if(n.querySelector("#analysis-count").textContent=String(o.length),o.length===0){t.innerHTML=`
  ...
  `,a.appendChild(u),document.body.appendChild(a),u.querySelector(".close-analysis-btn")?.addEventListener("click",()=>a.remove()),a.addEventListener("click",s=>{s.target===a&&a.remove()});const h=window.currentProjectId||document.body.dataset.projectId||n.closest("[data-project-id]")?.getAttribute("data-project-id")||"";u.querySelectorAll(".person-chip").forEach(s=>{const b=s;s.addEventListener("click",async $=>{$.stopPropagation();const k=b.dataset.name||"",f=b.dataset.role||"",v=b.dataset.org||"",p=b.dataset.contactId||"",w=b.dataset.contactName||"";await Q(b,k,f,v,h,p,w)})})}async function Q(n,e,t,a,o,i="",l=""){document.querySelectorAll(".person-link-menu").forEach(s=>s.remove());const r=!!i,c=document.createElement("div");c.className="person-link-menu",c.innerHTML=`
    <style>
      .person-link-menu {
        position: fixed;
        z-index: 10002;
  ...
  `;const m=n.getBoundingClientRect();c.style.position="fixed",c.style.top=`${Math.min(m.bottom+8,window.innerHeight-420)}px`,c.style.left=`${Math.min(m.left,window.innerWidth-320)}px`,document.body.appendChild(c);const u=c.querySelector("input");u?.focus();try{const b=(await x.get(`/api/contacts?project_id=${o}&limit=100`)).data.contacts||[],$=c.querySelector(".person-link-menu-list"),k=f=>{if(f.length===0){$.innerHTML='<div class="person-link-menu-empty">No matching contacts</div>';return}$.innerHTML=f.map(v=>{const p=(v.name||"?").split(" ").map(T=>T[0]).slice(0,2).join("").toUpperCase(),w=v.avatar_url||v.photo_url||"",_=v.role||"";return`
  ...
  `}async function P(n,e){const t=n.querySelector("#versions-list");try{const o=(await x.get(`/api/documents/${e}/versions`)).data.versions||[];if(n.querySelector("#versions-count").textContent=String(o.length||1),o.length===0){t.innerHTML=`
    ...
    `).join("")}catch{t.innerHTML='<div class="empty-section">Version history not available</div>'}}async function F(n,e){const t=n.querySelector("#activity-timeline");try{const o=(await x.get(`/api/documents/${e}/activity`)).data.activities||[];if(o.length===0){t.innerHTML='<div class="empty-section">No activity recorded</div>';return}t.innerHTML=o.map(i=>`
    ...
    `).join("")}catch{t.innerHTML='<div class="empty-section">Activity not available</div>'}}async function Y(n,e){const t=n.querySelector("#notes-container"),a=n.querySelector("#notes-tab");try{const i=(await x.get(`/api/documents/${e}/extraction`)).data.extraction;if(!(i?.notes_rendered_text||(i?.notes?.key_points?.length||0)>0)){a&&a.classList.add("hidden");return}a&&a.classList.remove("hidden");const r=i?.notes_rendered_text||"",c=i?.notes;if(r)t.innerHTML=`
        <style>
          .notes-rendered {
            padding: 20px;
            background: var(--bg-secondary);
