...
class TeamAnalyzer {
    ...
    async resolveProviderConfig(provider, providerConfig = {}) {
        if (!provider || provider === 'ollama') return providerConfig;
        if (providerConfig.apiKey) return providerConfig;

        // Centralized secrets lookup (system scope)
        try {
    ...
    async runFullAnalysis(allInterventions, personName, relationshipContext, objective, existingProfile) {
        ...
        const resolvedProviderConfig = await this.resolveProviderConfig(provider, providerConfig);
        ...
        for (const doc of documents) {
            if (!doc.content && doc.filepath) {
                try {
                    ...
                    // Google Drive-backed file reference (filepath = gdrive:<fileId>)
                    if (typeof doc.filepath === 'string' && doc.filepath.startsWith('gdrive:')) {
                        try {
                            ...
                            if (fileId) {
                                ...
                                if (driveClient?.success) {
                                    ...
                                    if (dl?.success && dl.buffer) {
                                        // Best-effort: assume UTF-8 text; if it's binary, this will be garbage but still non-crashing.
                                        doc.content = dl.buffer.toString('utf-8');
                                        console.log(`[TeamAnalyzer] Loaded content from Google Drive: ${fileId} (${doc.content.length} chars)`);
                                    }
                                }
                            }
                        } catch (driveErr) {
                            console.warn(`[TeamAnalyzer] Could not load content from Google Drive: ${driveErr.message}`);
                        }
                    }
                    ...
                    if (!doc.content) {
                        ...
                        for (const fpath of possiblePaths) {
                            if (fpath && typeof fpath === 'string' && fs.existsSync(fpath)) {
